...
- (void)setupDownRefresh
{
    ...
    [control addTarget:self action:@selector(getMediaTimeline:) forControlEvents:UIControlEventValueChanged];
    ...
    // 3.马上加载数据
    [self getMediaTimeline:control];
}
- (void)getMediaTimeline:(UIRefreshControl *)control

{
    ...
    [self.account.twitter verifyCredentialsWithUserSuccessBlock:^(NSString *username, NSString *userID) {
        ...
        [self.account.twitter getUserTimelineWithScreenName:scrname sinceID:params[@"since_id"] maxID:nil count:50 successBlock:^(NSArray *statuses) {
            
            //mj_objectArrayWithKeyValuesArray
            [statuses enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                
                if ([[obj allKeys] containsObject:@"extended_entities.media[0].media_url"]) {
...
- (void)loadMoreStatus
{
    ...
    [self.account.twitter verifyCredentialsWithUserSuccessBlock:^(NSString *username, NSString *userID) {
        ...
        [self.account.twitter getStatusesUserTimelineForUserID:userID screenName:nil sinceID:nil count:@"50" maxID:params[@"max_id"] trimUser:nil excludeReplies:nil contributorDetails:nil includeRetweets:nil successBlock:^(NSArray *statuses) {
            [statuses enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                
                if ([[obj allKeys] containsObject:@"extended_entities"]) {
                    NSArray *newStatuses = [Tweet mj_objectArrayWithKeyValuesArray:statuses];
